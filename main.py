...
async def change_price():
    ...
    a.ylim([min(btc)-min(btc)/500, max(btc)+max(btc)/500])
    a.xlabel('시간 (h)')
    a.tick_params(axis='y', length=0, labelcolor='white')
    plt.title('주식 그래프', loc='right')
    ax2 = ax1.twinx()
    b = ax2.plot(time, eth, color='skyblue', label="이더리움")
    b.ylim([min(eth)-min(eth)/500, max(eth)+max(eth)/500])
    b.tick_params(axis='y', length=0, labelcolor='white')
    ax3 = ax2.twinx()
    c = ax3.plot(time, ltc, color='royalblue', label="라이트코인")
    c.ylim([min(ltc)-min(ltc)/500, max(ltc)+max(ltc)/500])
    c.tick_params(axis='y', length=0, labelcolor='white')
    ax4 = ax3.twinx()
    d = ax4.plot(time, dot, color='mediumvioletred', label="폴카닷")
    d.ylim([min(dot)-min(dot)/500, max(dot)+max(dot)/500])
    d.tick_params(axis='y', length=0, labelcolor='white')
    ax5 = ax4.twinx()
    e = ax5.plot(time, ada, color='midnightblue', label="에이다")
    e.ylim([min(ada)-min(ada)/500, max(ada)+max(ada)/500])
    e.tick_params(axis='y', length=0, labelcolor='white')
    ax6 = ax5.twinx()
    f = ax6.plot(time, doge, color='gold', label="도지코인")
    f.ylim([min(doge)-min(doge)/500, max(doge)+max(doge)/500])
    f.tick_params(axis='y', length=0, labelcolor='white')
