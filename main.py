...
async def change_price():
    ...
    plt.ylim([min(btc)-min(btc)/100, max(btc)+max(btc)/100])
    plt.xlabel('시간 (h)')
    plt.tick_params(axis='y', length=0, labelcolor='white')
    plt.legend(loc=(0.02, 0.92))
    plt.twinx()
    plt.plot(time, eth, color='skyblue', label="이더리움")
    plt.ylim([min(eth)-min(eth)/100, max(eth)+max(eth)/100])
    plt.tick_params(axis='y', length=0, labelcolor='white')
    plt.legend(loc=(0.02, 0.85))
    plt.twinx()
    plt.plot(time, ltc, color='royalblue', label="라이트코인")
    plt.ylim([min(ltc)-min(ltc)/100, max(ltc)+max(ltc)/100])
    plt.tick_params(axis='y', length=0, labelcolor='white')
    plt.legend(loc=(0.02, 0.78))
    plt.twinx()
    plt.plot(time, dot, color='mediumvioletred', label="폴카닷")
    plt.ylim([min(dot)-min(dot)/100, max(dot)+max(dot)/100])
    plt.tick_params(axis='y', length=0, labelcolor='white')
    plt.legend(loc=(0.02, 0.71))
    plt.twinx()
    plt.plot(time, ada, color='midnightblue', label="에이다")
    plt.ylim([min(ada)-min(ada)/100, max(ada)+max(ada)/100])
    plt.tick_params(axis='y', length=0, labelcolor='white')
    plt.legend(loc=(0.02, 0.64))
    plt.twinx()
    plt.plot(time, doge, color='gold', label="도지코인")
    plt.ylim([min(doge)-min(doge)/100, max(doge)+max(doge)/100])
    plt.tick_params(axis='y', length=0, labelcolor='white')
    plt.legend(loc=(0.02, 0.57))
