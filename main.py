...
async def change_price():
    ...
    ax1.ylim([min(btc)-min(btc)/500, max(btc)+max(btc)/500])
    ax1.xlabel('시간 (h)')
    ax1.tick_params(axis='y', length=0, labelcolor='white')
    ax1.title('주식 그래프', loc='right')
    ax2 = ax1.twinx()
    ax2.plot(time, eth, color='skyblue', label="이더리움")
    ax2.ylim([min(eth)-min(eth)/500, max(eth)+max(eth)/500])
    ax2.tick_params(axis='y', length=0, labelcolor='white')
    ax2.legend(loc=(0.01, 0.85), framealpha=0.8)
    ax3 = ax2.twinx()
    ax3.plot(time, ltc, color='royalblue', label="라이트코인")
    ax3.ylim([min(ltc)-min(ltc)/500, max(ltc)+max(ltc)/500])
    ax3.tick_params(axis='y', length=0, labelcolor='white')
    ax3.legend(loc=(0.01, 0.78), framealpha=0.8)
    ax4 = ax3.twinx()
    ax4.plot(time, dot, color='mediumvioletred', label="폴카닷")
    ax4.ylim([min(dot)-min(dot)/500, max(dot)+max(dot)/500])
    ax4.tick_params(axis='y', length=0, labelcolor='white')
    ax4.legend(loc=(0.01, 0.71), framealpha=0.8)
    ax5 = ax4.twinx()
    ax5.plot(time, ada, color='midnightblue', label="에이다")
    ax5.ylim([min(ada)-min(ada)/500, max(ada)+max(ada)/500])
    ax5.tick_params(axis='y', length=0, labelcolor='white')
    ax5.legend(loc=(0.01, 0.64), framealpha=0.8)
    ax6 = ax5.twinx()
    ax6.plot(time, doge, color='gold', label="도지코인")
    ax6.ylim([min(doge)-min(doge)/500, max(doge)+max(doge)/500])
    ...
    ax1.legend(h1+h2+h3+h4+h5+h6, l1+l2+l3+l4+l5+l6)
    ax1.savefig('graph.png')
